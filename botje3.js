...
document.addEventListener('DOMContentLoaded', function() {
    console.log('🚀 Chatbot geladen - versie ' + BOTJE_VERSION + ' (Met Service Worker Background Requests)');
    ...
    if (savedWebhookOverride) {
        ...
        console.log('🔧 Webhook override hersteld:', webhookUrlOverride);
    }
    ...
    setTimeout(function() {
        console.log('🔄 Delayed customer data check...');
        loadCustomerData();
        updateDebugPanel();
    }, 1000);
...
function initDebugMode() {
    ...
    if (debugParam === 'true' || debugParam === '1' || debugParam === '') {
        ...
        console.log('🐛 Debug mode ACTIEF (sessie)');
        document.getElementById('debug-toggle').style.display = 'block';
    }
...
async function initServiceWorker() {
    if (!('serviceWorker' in navigator)) {
        console.warn('⚠️ Service Workers niet ondersteund in deze browser');
        return;
    }
...
function handleServiceWorkerMessage(event) {
    ...
    console.log('📬 Bericht van Service Worker:', type, event.data);
    ...
    if (type === 'CHAT_RESPONSE') {
        ...
        if (status === 'completed' && response) {
            ...
            if (shouldReloadCart) {
                console.log('🛒 Webhook vraagt om cart reload');
                setTimeout(function() {
                    const success = reloadMagentoCart();
                    if (success) {
                        addMessage('✅ Winkelwagen is bijgewerkt!', 'bot');
                    }
                }, 1000);
            }
...
function checkPendingResponses() {
    if (navigator.serviceWorker.controller) {
        console.log('🔍 Checking voor pending responses...');
...
function handlePendingStatus(pendingResponse, pendingRequest) {
    console.log('📋 Pending status:', { pendingResponse, pendingRequest });
    
    // Er is een ongelezen response
    if (pendingResponse && !pendingResponse.read) {
        console.log('📬 Ongelezen response gevonden!');
    ...
    if (pendingRequest && pendingRequest.status === 'pending') {
        ...
        if (age < maxAge) {
            console.log('⏳ Request nog in behandeling:', Math.round(age / 1000) + 's oud');
            showTyping();
            showPendingIndicator();
        } else {
            console.log('⏱️ Request is verlopen, wissen...');
            clearPendingData();
        }
    }
}
...
function updateChatBadge(show) {
    ...
    try {
        ...
        console.warn('⚠️ Kon badge status niet opslaan');
    }
}
...
function restoreBadgeStatus() {
    ...
    try {
        ...
        if (hasBadge) {
            console.log('🔔 Badge status hersteld: actief');
            updateChatBadge(true);
        }
    } catch (e) {
        console.warn('⚠️ Kon badge status niet herstellen');
    }
}
    ...
    try {
        ...
        if (userEmail) {
            if (userEmail.endsWith('@cartid.nl')) {
                console.log('💾 Chat opgeslagen (email van Cart ID)');
            } else {
                console.log('💾 Chat opgeslagen (echt email adres)');
            }
        } else {
            console.log('💾 Chat opgeslagen (geen email)');
        }
    } catch (e) {
        console.warn('⚠️ Kon chat niet opslaan:', e.message);
    }
}
...
function restoreChat() {
    try {
        ...
        if (!saved) {
            console.log('ℹ️ Geen opgeslagen chat gevonden');
            return false;
        }
        ...
        if (Date.now() - chatData.timestamp > maxAge) {
            console.log('ℹ️ Opgeslagen chat is te oud, wordt verwijderd');
            localStorage.removeItem('elektramat_chat');
            return false;
        }
        
        console.log('✅ Chat hersteld:', chatData.chatId);
        ...
        console.warn('⚠️ Kon chat niet herstellen:', e.message);
        localStorage.removeItem('elektramat_chat');
        return false;
    }
}
...
function clearChat() {
    try {
        localStorage.removeItem('elektramat_chat');
        console.log('🗑️ Chat verwijderd uit opslag');
    } catch (e) {
        console.warn('⚠️ Kon chat niet verwijderen:', e.message);
    }
}

function loadCustomerData() {
    // Try to get cart ID from Hyvä (Magento 2 Hyvä theme)
    magentoCartId = getHyvaCartId() || getCookie('magento_cart_id') || '';
    customerLoggedIn = isCustomerLoggedIn();
    
    console.log('📊 Customer data loaded:');
    console.log('  🛒 Cart ID:', magentoCartId || '(geen cart)');
    console.log('  🔐 Ingelogd:', customerLoggedIn ? 'Ja' : 'Nee');
    ...
    console.log('  💬 Chat actief:', chatIsActive ? 'Ja' : 'Nee');
    console.log('  📧 Email form zichtbaar:', emailFormVisible ? 'Ja' : 'Nee');
    ...
    if (userEmail && userEmail.endsWith('@cartid.nl') && !customerLoggedIn) {
        console.warn('⚠️ Klant was ingelogd met Cart ID email maar is nu uitgelogd');
        console.log('🔄 Reset email en toon formulier');
        userEmail = null;
    ...
    if (chatIsActive) {
        console.log('✅ Chat is al actief, geen wijzigingen');
    ...
    if (customerLoggedIn) {
        console.log('✅ Klant is ingelogd, email formulier overslaan');
        document.getElementById('email-form').style.display = 'none';
        document.getElementById('email-form').classList.remove('active');
        document.getElementById('chat-input-area').classList.add('active');
        ...
        if (!userEmail) {
            ...
            if (customerEmail) {
                ...
                console.log('✅ Customer email gevonden:', userEmail);
                ...
                console.log('ℹ️ Ingelogde klant zonder email, gebruik Cart ID als email:', userEmail);
            } else {
                // No email and no cart ID for logged-in customer, leave empty
                userEmail = '';
                console.log('⚠️ Ingelogde klant zonder email en zonder Cart ID, veld blijft leeg');
            }
        }
    } else {
        // Customer is NOT logged in
        console.log('ℹ️ Klant is niet ingelogd');
        
        // Make sure email form is visible if no email is set
        if (!userEmail) {
            console.log('📧 Geen email ingevuld, toon formulier');
            document.getElementById('email-form').style.display = 'block';
            document.getElementById('email-form').classList.add('active');
            document.getElementById('chat-input-area').classList.remove('active');
        } else {
            console.log('📧 Email al ingevuld:', userEmail);
...
function getCustomerEmail() {
    try {
        ...
        try {
            ...
            console.log('ℹ️ localStorage email check overgeslagen');
        }
        ...
        try {
            ...
            console.log('ℹ️ Magento customerData email check overgeslagen');
        }
        
        return null;
    } catch (e) {
        console.warn('⚠️ Error bij ophalen customer email:', e.message);
        return null;
    }
}
...
function attachEvents() {
    ...
    document.getElementById('menu-reload-cart').onclick = function() {
        ...
        if (success) {
            alert('✅ Winkelwagen wordt herlaaden...');
        } else {
            if (confirm('⚠️ Automatisch herladen niet mogelijk.\n\nWil je de pagina verversen?')) {
                reloadCartAndPage();
            }
        }
    };
    ...
    document.getElementById('menu-clear-chat').onclick = function() {
        ...
        if (confirm('Chat geschiedenis wissen? Dit kan niet ongedaan gemaakt worden.')) {
            clearChat();
            alert('✅ Chat geschiedenis is gewist');
        }
    };
...
function toggleMenu() {
    ...
    console.log('📋 Menu:', menu.classList.contains('active') ? 'open' : 'closed');
}

function toggleFullscreen() {
    const container = document.getElementById('chat-container');
    container.classList.toggle('fullscreen');
    console.log('🖥️ Fullscreen:', container.classList.contains('fullscreen'));
}
...
function startNewChat() {
    if (confirm('Nieuwe chat starten? De huidige chat wordt verwijderd.')) {
        ...
        if (customerLoggedIn) {
            console.log('✅ Nieuwe chat gestart (ingelogd)');
            document.getElementById('email-form').style.display = 'none';
            document.getElementById('chat-input-area').classList.add('active');
            ...
            console.log('ℹ️ Nieuwe chat gestart (niet ingelogd)');
            userEmail = null;
            document.getElementById('email-form').classList.add('active');
            document.getElementById('chat-input-area').classList.remove('active');
        }
...
async function sendToWebhook(message, file, isRetry = false) {
    ...
    try {
        ...
        if (userEmail && userEmail.endsWith('@cartid.nl') && !customerLoggedIn) {
            console.error('❌ SECURITY: Cart ID email detected for non-logged-in customer!');
        ...
        console.log('🚀 POST to webhook:', activeUrl);
        console.log('🔧 Test mode:', isTestMode() ? 'JA' : 'NEE');
        console.log('🔧 Service Worker:', serviceWorkerReady ? 'JA' : 'NEE');
        ...
        if (serviceWorkerReady && navigator.serviceWorker.controller && !file) {
            ...
            console.log('📤 Request via Service Worker:', requestId);
        ...
        if (!res.ok) {
            ...
            if (res.status === 503) {
                if (retryCount < MAX_RETRIES && !isRetry) {
                    retryCount++;
                    hideTyping();
                    showError('⏳ Server bezet, probeer opnieuw... (poging ' + retryCount + '/' + MAX_RETRIES + ')');
        ...
        if (shouldReloadCart) {
            ...
            setTimeout(function() {
                ...
                if (success) {
                    ...
                    addMessage('ℹ️ Tip: Ververs de pagina om je winkelwagen bij te werken.', 'bot');
                }
            }, 1000);
        }
        ...
        if (err.name === 'AbortError') {
            showError('⏱️ Request timeout na ' + (REQUEST_TIMEOUT / 1000) + ' seconden. Server reageert niet.');
...
function refreshMagentoCart() {
    console.log('🔄 Winkelwagen herladen...');
    ...
    try {
        ...
        if (typeof window.customerData !== 'undefined' && window.customerData.reload) {
            console.log('✅ Methode 1: Magento customerData reload');
            window.customerData.reload(['cart'], false);
            return true;
        }
        
        // Method 2: Hyvä theme Alpine.js event dispatch
        if (typeof window.Alpine !== 'undefined') {
            console.log('✅ Methode 2: Hyvä Alpine.js event');
        ...
        if (typeof window.customerData !== 'undefined' && window.customerData.invalidate) {
            console.log('✅ Methode 3: Magento customerData invalidate');
            window.customerData.invalidate(['cart']);
            window.customerData.reload(['cart'], false);
            return true;
        }
        
        // Method 4: Direct localStorage manipulation (Hyvä fallback)
        try {
            console.log('ℹ️ Methode 4: localStorage direct update');
            ...
            if (storage) {
                ...
                // Trigger storage event for Hyvä
            ...
            console.warn('⚠️ localStorage update failed:', e.message);
        }
        
        console.warn('⚠️ Geen werkende methode gevonden voor cart reload');
        return false;
        
    } catch (e) {
        console.error('❌ Error bij cart reload:', e.message);
        return false;
    }
}

function reloadCartAndPage() {
    console.log('🔄 Volledige pagina reload (met cart update)');
...
function getHyvaCartId() {
    try {
        // Check if Hyvä theme is available
        if (typeof hyva === 'undefined' || !hyva.getBrowserStorage) {
            console.log('ℹ️ Hyvä theme niet gevonden, gebruik fallback');
            return null;
        }
        
        // Get cart ID from Hyvä browser storage
        const storage = hyva.getBrowserStorage();
        if (!storage || !storage['mage-cache-storage']) {
            console.log('ℹ️ Geen Magento cache storage gevonden');
            return null;
        }
        
        const cacheData = JSON.parse(storage['mage-cache-storage']);
        if (cacheData && cacheData.cart && cacheData.cart.cartId) {
            console.log('✅ Hyvä Cart ID gevonden:', cacheData.cart.cartId);
            return cacheData.cart.cartId;
        }
        
        return null;
    } catch (e) {
        console.warn('⚠️ Error bij ophalen Hyvä cart ID:', e.message);
        return null;
    }
}
    ...
    try {
            ...
            if (storage && storage['mage-cache-storage']) {
                ...
                if (cacheData && cacheData.customer) {
                    ...
                    if (cacheData.customer.firstname || cacheData.customer.lastname) {
                        console.log('✅ Klant is ingelogd (Hyvä customer naam gevonden)');
                        return true;
                    }
                }
                ...
                if (cacheData && cacheData.customer_logged_in === true) {
                    console.log('✅ Klant is ingelogd (customer_logged_in flag)');
                    return true;
                }
            }
        }
            ...
            if (localCache) {
                ...
                if (cacheData && cacheData.customer) {
                    if (cacheData.customer.firstname) {
                        console.log('✅ Klant is ingelogd (localStorage)');
                        return true;
                    }
                }
            }
        } catch (e) {
            console.log('ℹ️ localStorage check overgeslagen');
        }
        
        // Method 3: Check cookies
        const loggedInCookie = getCookie('customer_logged_in');
        if (loggedInCookie === '1' || loggedInCookie === 'true') {
            console.log('✅ Klant is ingelogd (cookie)');
            return true;
        }
        ...
        try {
            if (typeof window.customerData !== 'undefined' && window.customerData.get) {
                ...
                if (customer && customer.firstname) {
                    console.log('✅ Klant is ingelogd (Magento customerData)');
                    return true;
                }
            }
        } catch (e) {
            console.log('ℹ️ Magento customerData check overgeslagen');
        }
        ...
        try {
            ...
            if (pageText.includes('Ingelogd als') || pageText.includes('Uitloggen')) {
                console.log('✅ Klant is ingelogd (pagina tekst)');
                return true;
            }
        } catch (e) {
            console.log('ℹ️ Pagina tekst check overgeslagen');
        }
        
        console.log('❌ Klant is niet ingelogd');
        return false;
    } catch (e) {
        console.warn('⚠️ Error bij checken login status:', e.message);
        return false;
    }
}
...
function getCurrentPageInfo() {
    ...
    console.log('📄 Pagina info:', pageInfo);
    return pageInfo;
}
...
function detectPageType() {
    ...
    // Magento/Hyvä specifieke pagina detectie
...
function getProductInfo() {
    ...
    try {
        ...
        console.warn('⚠️ Kon product info niet ophalen:', e.message);
        return null;
    }
}
...
function getCategoryInfo() {
    ...
    try {
        ...
        console.warn('⚠️ Kon categorie info niet ophalen:', e.message);
        return null;
    }
}
...
function updateDebugPanel() {
    ...
    loggedInEl.textContent = customerLoggedIn ? 'JA ✓' : 'NEE';
    ...
    const cartSource = debugCartIdOverride ? '(override)' : magentoCartId ? (getHyvaCartId() ? '(Hyvä)' : '(cookie)') : '';
    ...
    if (isTestMode()) {
        ...
        webhookEl.textContent = 'productie ✓';
...
function overrideCartId() {
    ...
    updateDebugPanel();
    alert(debugCartIdOverride ? '✅ Cart ID ingesteld: ' + debugCartIdOverride : 'Cart ID gewist');
}

function overrideWebhookUrl() {
    const newUrl = document.getElementById('debug-webhook-override').value.trim();
    if (!newUrl) {
        alert('⚠️ Voer een webhook URL in');
        return;
    }
    
    // Basic URL validation
    if (!newUrl.startsWith('http://') && !newUrl.startsWith('https://')) {
        alert('⚠️ URL moet beginnen met http:// of https://');
        return;
    }
    ...
    console.log('🔧 Webhook override ingesteld:', newUrl);
    alert('✅ Webhook URL ingesteld!\n\nTest mode is nu ACTIEF.\nAlle berichten gaan naar:\n' + newUrl);
}
...
function clearWebhookOverride() {
    ...
    console.log('🔧 Webhook override verwijderd, terug naar productie');
    alert('✅ Terug naar productie!\n\nBerichten gaan nu naar:\n' + WEBHOOK_URL_PRODUCTION);
}
...
function updateTestModeBanner() {
    ...
    if (isTestMode()) {
        ...
        if (menuStatus) {
            menuStatus.textContent = '⚠️ TEST MODE';
...
function refreshCartId() {
    ...
    console.log('🔄 Refreshing cart and login data...');
    loadCustomerData();
    ...
    if (oldCartId !== magentoCartId) {
        changes.push('Cart: ' + (oldCartId || '(leeg)') + ' → ' + (magentoCartId || '(leeg)'));
    }
    if (oldLoggedIn !== customerLoggedIn) {
        changes.push('Login: ' + (oldLoggedIn ? 'Ja' : 'Nee') + ' → ' + (customerLoggedIn ? 'Ja' : 'Nee'));
    }
    
    if (changes.length > 0) {
        alert('🔄 Data bijgewerkt!\n\n' + changes.join('\n'));
        console.log('🔄 Data refreshed:', { cartId: magentoCartId, loggedIn: customerLoggedIn });
    } else {
